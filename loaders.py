...
class ICLDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ## Generate a sequence of shape (seq_len, x_dim) from the uniform distribution
        Xs = np.random.uniform(low=-1, high=1, size=(self.seq_len, self.x_dim)).astype(np.float32)
        ...
        # output_matrix = np.clip(output_matrix, -1, -1)

        ## Actually, sample output_matrix from a uniform distribution
        output_matrix = np.random.uniform(low=-1, high=1, size=(self.x_dim,)).astype(np.float32)
        ...
        ys = (Xs**1)@output_matrix  ## Shape: (seq_len, x_dim)
        # print("Shapes before multiplication:", output_matrix.shape, inputs.shape, outputs.shape, flush=True)

        ys_zero = ys.copy()
        ys_zero[-1] = 0  ## Set the first output to 0

        ## Do a cumsum on the ys to get new ys: TODO?
        ys = np.cumsum(ys, axis=-1)  ## Shape: (seq_len, x_dim)
        ys_zero = np.cumsum(ys_zero, axis=-1)  ## Shape: (seq_len, x_dim)
        ## Do a cumsum on the Xs along the features
        Xs = np.cumsum(Xs, axis=-1)  ## Shape: (seq_len, x_dim)
        ...
        # ## The repeat method above works if Xs is unidimensional, but here we want to keep the x_dim. We want to turn a vector of shape (seq_len, x_dim) into a matrix of shape (seq_len, seq_len, x_dim)
        # Xs = np.repeat(Xs[:, :, None], self.seq_len, axis=1)  ## Shape: (seq_len, seq_len, x_dim)
        # ## Colalpse the last two dimensions to have a shape of (seq_len, seq_len*x_dim)
        # Xs = Xs.reshape(self.seq_len, -1)  ## Shape: (seq_len, seq_len*x_dim)
        ...
        outputs = np.concatenate((Xs, ys_zero[:, None]), axis=-1)    # Shape: (seq_len, x_dim + 1)
        # outputs = np.concatenate((Xs, np.cumsum(ys[:, None], axis=-1)), axis=-1)    # Shape: (seq_len, x_dim + 1)
        ...
        inputs = outputs.copy()
        inputs[-1, -1] = 0
        outputs_1D = ys[:, None]  ## Shape: (seq_len,)

        # ## In the last row, only keep 5 consecutive values, the rest is set to 0 (The initilal net doesn't have the ground thruth)
        # start = 0
        # end = -1
        # inputs[:start, -1] = inputs[start, -1] 
        # # inputs[end:, -1] = 0
        # inputs[1::2, -1] = 0        ## Every two steps, we set the last value to 0
        # # print("Inputs:", inputs[:, -1], flush=True)

        # ## 1D output to use
        # outputs_1D = ys[:, None]  ## Shape: (seq_len,)
        # outputs_1D[:start, 0] = outputs_1D[start, 0]
        # # outputs_1D[end:, 0] = 0
        # # outputs[:start, -1] = 0
        ...
        return (inputs, self.t_eval), outputs_1D
...
#     This class is refactored to use the data generation logic from the paper's
#     official setting, while maintaining the requested input/output format.
#     """
#     def __init__(self, x_dim=2, seq_len=32, num_envs=1024, positional_enc=None, white_cov=False):
#         """
#         Initializes and pre-generates the entire dataset.
        
#         Note: The 'positional_enc' argument is ignored to match the paper's
#         theoretical setup, which does not use positional encodings.
#         """
#         self.x_dim = x_dim          # Corresponds to D in the paper
#         self.seq_len = seq_len      # Corresponds to N+1 in the paper
#         self.total_envs = 1024*1  # Corresponds to P (number of samples)
#         # self.total_envs = num_envs  # Corresponds to P (number of samples)
#         self.out_dim = 1            # Linear regression target is a scalar

#         # Generate t_eval as in your original code, without positional encodings
#         self.t_eval = np.linspace(0., 1., seq_len)[:, None].astype(np.float32)
        
#         # --- Internal Data Generation (from the paper's logic) ---
#         cov_matrix = self._gen_cov(white_cov)
#         self._gen_data(cov_matrix)
#         # The generated data is stored in self.inputs and self.outputs
#         # ---------------------------------------------------------

#     def _gen_cov(self, white_cov):
#         """Generates the D x D covariance matrix Lambda for input tokens x."""
#         if white_cov:
#             return np.eye(self.x_dim)
#         else:
#             eigval = 1 / (np.arange(self.x_dim) + 1)
#             Q, _ = np.linalg.qr(np.random.randn(self.x_dim, self.x_dim))
#             Lambda = Q @ np.diag(eigval) @ Q.T
#             return Lambda / np.trace(Lambda)

#     def _whiten(self, W):
#         """Whitens a batch of task vectors W."""
#         W = W - torch.mean(W, dim=0, keepdim=True)
#         cov_w = torch.cov(W.T)
#         eigvals, eigvecs = torch.linalg.eigh(cov_w)
#         whitening_matrix = eigvecs @ torch.diag(1.0 / torch.sqrt(eigvals + 1e-6)) @ eigvecs.T
#         return W @ whitening_matrix.T

#     def _gen_data(self, cov_matrix):
#         """Generates and stores the full dataset of inputs and outputs."""
#         # Generate x tokens: (num_samples, seq_len, x_dim)
#         x = torch.from_numpy(
#             np.random.multivariate_normal(np.zeros(self.x_dim), cov_matrix, size=(self.total_envs, self.seq_len))
#         ).float()

#         # Generate unique, whitened task vectors w for each sequence (pure ICL)
#         w_ic = torch.normal(0, 1, size=(self.total_envs, self.x_dim))
#         w_ic = self._whiten(w_ic).unsqueeze(-1) # Shape: (num_samples, x_dim, 1)

#         # Calculate labels y = w^T * x for each sequence
#         y = torch.einsum('nsi,nio->nso', x, w_ic) # Shape: (num_samples, seq_len, 1)

#         # The full sequence is the concatenation of x and y
#         full_sequence = torch.cat((x, y), dim=2) # Shape: (num_samples, seq_len, x_dim + 1)

#         # The model's input has the final label zeroed out
#         # self.inputs = full_sequence.clone()
#         self.inputs = full_sequence.clone().numpy()
#         self.inputs[:, -1, -1] = 0 # Zero out y_q for every sequence

#         # The output target is the full sequence of y-values
#         # self.outputs = full_sequence[:, :, -1:].clone() # Shape: (num_samples, seq_len, 1)
#         self.outputs = full_sequence[:, :, -1:].clone().numpy() # Shape: (num_samples, seq_len, 1)
...
#     def __getitem__(self, idx):
#         """
#         Returns a tuple of (inputs, t_eval), and outputs_1D for a given index.
#         This matches your requested format.
#         """
#         inputs = self.inputs[idx]
#         outputs_1D = self.outputs[idx]

#         ## We want to turn inputs of shape (seq_len, x_dim) into a matrix of shape (seq_len, seq_len, x_dim), then (seq_len, seq_len*x_dim)
#         # Xs = np.repeat(Xs[:, :, None], self.seq_len, axis=1)  ## Shape: (seq_len, seq_len, x_dim)
#         # ## Colalpse the last two dimensions to have a shape of (seq_len, seq_len*x_dim)
#         # Xs = Xs.reshape(self.seq_len, -1)  ## Shape: (seq_len, seq_len*x_dim)

#         # ## We want to turn inputs of shape (seq_len, x_dim+1) into a matrix of shape (seq_len, seq_len, x_dim+1), then (seq_len, seq_len*(x_dim+1))
#         # inputs = np.repeat(inputs[:, :, None], self.seq_len, axis=1)  ## Shape: (seq_len, seq_len, x_dim+1)
#         # ## Collapse the last two dimensions to have a shape of (seq_len, seq_len*(x_dim+1))
#         # inputs = inputs.reshape(self.seq_len, -1)  ## Shape: (seq_len, seq_len*(x_dim+1))

#         # ## We want to turn inputs of shape (seq_len, x_dim) into a matrix of shape (seq_len, seq_len, x_dim), then (seq_len, seq_len*(x_dim))
#         # last_feature = inputs[:, -1:]  ## Shape: (seq_len, 1)
#         # inputs = np.repeat(inputs[:, :-1, None], self.seq_len, axis=1)  ## Shape: (seq_len, seq_len, x_dim)
#         # inputs = inputs.reshape(self.seq_len, -1)  ## Shape: (seq_len, seq_len*(x_dim))
#         # inputs = np.concatenate((inputs, last_feature), axis=-1)  ## Shape: (seq_len, seq_len*x_dim + 1)
...
def make_dataloaders(data_folder, config):
    ...
    if dataset in ["mnist", "mnist_fashion"]:
        ...
        # data_size = data_size*seq_length + 1  ## +1 for the output
        # # data_size = (data_size+1)*seq_length  ## +1 for the output
        # # exit(0)
